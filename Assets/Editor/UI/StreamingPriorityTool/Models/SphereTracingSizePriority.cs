...
{
    public class SphereTracingSizePriority : PriorityAlgorithm
    {
        ...
        public override List<GameObject> Sort(List<GameObject> assets, GameObject entryPoint, List<GameObject> except)
        {
            ...
            for (int i = 0; i < except.Count(); i++)
                govalues[i] = new GOValue(except[i], float.MinValue+i); // the amount of hits are always non-negative hence -1 is always smaller
            ...
            var onSightContainer= GetOnSightSizePriority(cam);
            ...
            GOValue[] onSight = onSightContainer.Values.ToArray(); Utilities.Paint(onSight, Color.green, Color.red); Debug.Log($"OFFSET: {offset}");
            ...
            Func<IEnumerable<GameObject>> extract = () => 
            {
                List<GameObject> notOnSight = new List<GameObject>();
                foreach (var go in onSightContainer.Values) notOnSight.Add(go.obj);
                return notOnSight;
            };

            GOValue[] occludedOnCamera = Evaluate(onCamera.Except(extract())); Utilities.Paint(occludedOnCamera, Color.cyan, Color.blue); Debug.Log($"OFFSET: {offset}");
        ...
        private GOValue[] Evaluate(IEnumerable<GOValue> assets)
        {
            GOValue[] arr = new GOValue[assets.Count()];
            int index = 0;
            foreach(var go in assets)
            {
                // let's recycle memory for the environment
                go.value = (Distance(go.obj) + offset) / go.value;
                arr[index++] = go; 
            }
        ...
        private float ClosestPointToBounds(Vector3 fromPos, Renderer to) => (to.bounds.ClosestPoint(fromPos) - fromPos).magnitude;
